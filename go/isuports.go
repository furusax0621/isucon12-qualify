...
func playerHandler(c echo.Context) error {
	...
	if err := tenantDB.SelectContext(ctx, &c3, "select competition.title as competition_title, player_score.score from player_score join competition on competition_id = competition.id where competition.id IN ("+placeholder+")", args2...); err != nil {
		return fmt.Errorf("error retrieveCompetition: %w", err)
	}
